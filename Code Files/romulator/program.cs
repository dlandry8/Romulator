...
namespace Romulator {
	...
	class Program {
		...
		static void PrintIntro() {
			...
			Console.WriteLine("Copyright 2019 by Team Romulator");
		...
		static bool ReadData() {
			//filename.concat(".csv");
			using (FileStream fs = new FileStream(romulatorFile, FileMode.Open, FileAccess.Write)) {
				using (StreamReader recordStream = new StreamReader(fs)) {
					//bool thereIsData = true;
					//string lineOfData = "";
					//do {
						
					//} while (thereIsData == true);
				}
			}
		...
		static void Emulate() {
			...
			Console.WriteLine("Enter \"q\" or \"quit\" to stop emulating.");
